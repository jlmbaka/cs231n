...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    dists = (
      np.square(X).sum(axis=1)[:,np.newaxis] 
      - 2 * X.dot(self.X_train.T) 
      + np.square(self.X_train).sum(axis=1))
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      # Sort in ascending order, the indexes of distances
      # then get the indexes of the k smallest distances
      top_k_matches_idxs = np.argsort(dists[i,:])[:k]  
      closest_y = self.y_train[top_k_matches_idxs] # label of k nearest neighbours
